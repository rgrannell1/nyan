...
RAINBOW_END_ROW = 43
RAINBOW_ROW_OFFSET = (
    23  # vertical origin for mapping frame rows to rainbow stripe indices
)
...
ANIMATION_SPEED_DIVISOR = (
    2  # number of frames each stripe phase is held before advancing
)
...
def rainbow_color(col, row, frame_idx, scheme: ColourScheme):
    mod_col = (
        (-col + RAINBOW_COL_OFFSET) % RAINBOW_STRIPE_PERIOD
    ) // RAINBOW_STRIPE_HALF
...
def pixel_color(col, row, frame_idx, min_col, min_row, scheme: ColourScheme):
    ...
    if (
        actual_col < 0
        or actual_row < 0
        or actual_row >= FRAME_HEIGHT
        or actual_col >= FRAME_WIDTH
    ):
        return ","
...
class Nyancat:
    ...
    def _update_viewport(self):
        ...
        self.min_col, self.max_col, self.min_row, self.max_row = compute_viewport(
            height, width
        )
